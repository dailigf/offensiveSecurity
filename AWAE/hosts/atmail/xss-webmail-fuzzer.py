...
def craftHeaders(frmemail, dstemail, indexPayload, name , code, 
    ...
    if s_header and EMAIL_HEADERS.index(s_header) > 4:
        msg += "{}: {}\n".format(s_header, s_header.upper())
...
def sendData(conn, username, password, frmemail, dstemail, msg, mail_name):
    ...
    print("[+] Sending email {}".format(mail_name)) 
...
def showXSSPaylods(names, codes):
    for name, code in zip( names, codes ):
        print("[$] Payload {}  : {}".format(names.index(name), getTextFromXML(name)))
...
def main():
    ...
    if replay is not None:
        print("[+] Replaying payload {}".format(replay)) 
        name = names[replay]
        code = codes[replay]
